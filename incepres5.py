...
def stem(input):
    ...
    conv5_12 = tf.layers.conv1d(inputs=pool4_2, filters=64, kernel_size=41, strides=1,
                                padding='same', activation=tf.nn.relu, name='conv_%d' % 512)

    conv5_13 = tf.layers.conv1d(inputs=conv5_12, filters=96, kernel_size=41, strides=1,
...
def Incep_res_A(input, alpha=1.0):
    ...
    # input ?,568,392
    conv_1 = tf.layers.conv1d(inputs=input, filters=32, kernel_size=1, strides=1,
                               padding='same', activation=tf.nn.relu, name='A_conv_%d' % 1)
    # conv_a ?,568,32
    conv_21 = tf.layers.conv1d(inputs=input, filters=32, kernel_size=1, strides=1,
                               padding='same', activation=tf.nn.relu, name='A_conv_%d' % 21)

    conv_22 = tf.layers.conv1d(inputs=conv_21, filters=32, kernel_size=21, strides=1,
                               padding='same', activation=tf.nn.relu, name='A_conv_%d' % 22)
    # conv_22 ?,568,32
    conv_31 = tf.layers.conv1d(inputs=input, filters=32, kernel_size=1, strides=1,
                               padding='same', activation=tf.nn.relu, name='A_conv_%d' % 31)

    conv_32 = tf.layers.conv1d(inputs=conv_31, filters=48, kernel_size=41, strides=1,
                               padding='same', activation=tf.nn.relu, name='A_conv_%d' % 32)

    conv_33 = tf.layers.conv1d(inputs=conv_32, filters=64, kernel_size=41, strides=1,
    ...
    # conv_33 ?,568,64
    concat1 = tf.concat([conv_1,conv_22,conv_33, conv_42],2)
    # concat1 ?,622,128
    conv_5 = tf.layers.conv1d(inputs=concat1, filters=shape[2], kernel_size=1, strides=1,
                               padding='same', activation=None, name='A_conv_%d' % 5)
    # conv_4 ?,622,384
    output = tf.add((tf.multiply(input,alpha)),conv_5)
    # concat2 ?,622,384
...
def Incep_res_B(input,alpha=1.0):
    ...
    # conv1 ?,280,192
    # conv_21 = tf.layers.conv1d(inputs=input, filters=128, kernel_size=1, strides=1,
    #                            padding='same', activation=tf.nn.relu, name='B_conv_%d' % 21)

    conv_22 = tf.layers.conv1d(inputs=input, filters=192, kernel_size=11, strides=1,
                               padding='same', activation=tf.nn.relu, name='B_conv_%d' % 22)

    conv_32 = tf.layers.conv1d(inputs=input, filters=192, kernel_size=21, strides=1,
    ...
    conv_3 = tf.layers.conv1d(inputs=concat1, filters=shape[2], kernel_size=1, strides=1,
                               padding='same', activation=None, name='B_conv_%d' % 3)
    # conv3 307,1152
...
def Incep_res_C(input, alpha=1.0):
    ...
    conv_22 = tf.layers.conv1d(inputs=conv_21, filters=256, kernel_size=11, strides=1,
...
def Reduction_A(input):
    shape = input.get_shape().as_list()
    pool1 = tf.layers.max_pooling1d(inputs=input, pool_size=21, strides=2,
                                    padding='valid', name='AR_pool')
    # pool1 280,384
    conv1 = tf.layers.conv1d(inputs=input, filters=384, kernel_size=21, strides=2,
    ...
    conv22 = tf.layers.conv1d(inputs=conv21, filters=256, kernel_size=21, strides=1,
                              padding='same', activation=tf.nn.relu, name='RA_conv_%d' % 22)

    conv23 = tf.layers.conv1d(inputs=conv22, filters=384, kernel_size=21, strides=2,
...
def Reduction_B(input):
    ...
    # conv11 280,256
    conv12 = tf.layers.conv1d(inputs=conv11, filters=384, kernel_size=5, strides=2,
                              padding='valid', activation=tf.nn.relu, name='RB_conv_%d' % 12)
    # conv12 136,384
    conv21 = tf.layers.conv1d(inputs=input, filters=256, kernel_size=1, strides=1,
                             padding='same', activation=tf.nn.relu, name='RB_conv_%d' % 21)

    conv22 = tf.layers.conv1d(inputs=conv21, filters=256, kernel_size=11, strides=2,
    ...
    conv32 = tf.layers.conv1d(inputs=conv31, filters=256, kernel_size=21, strides=1,
                              padding='same', activation=tf.nn.relu, name='RB_conv_%d' % 32)

    conv33 = tf.layers.conv1d(inputs=conv32, filters=256, kernel_size=21, strides=2,
...
def make_net(input, keep_prob, channels, kernel_size, training=True, reg=0.01):
    ...
    output = tf.layers.dense(FC,shape_B[2]/2,activation=tf.nn.relu)
    output = tf.nn.dropout(output, keep_prob=keep_prob)
    output = tf.layers.dense(output, n_classes, activation=tf.nn.relu)
